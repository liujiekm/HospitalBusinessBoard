...
using HBB.DataContext.Properties;
...
namespace HBB.DataContext
{
	...
	public class OracleDatabase : Database
	{
		...
		private readonly IEnumerable<HBB.DataContext.IOraclePackage> packages;
		private static readonly IEnumerable<HBB.DataContext.IOraclePackage> emptyPackages = new List<HBB.DataContext.IOraclePackage>(0);
        ...
        /// </summary>
        /// <param name="connectionString">The connection string for the database.</param>
        /// <param name="packages">A list of <see cref="HBB.DataContext.IOraclePackage"/> objects.</param>
        public OracleDatabase(string connectionString, IEnumerable<HBB.DataContext.IOraclePackage> packages)
      ...
      return DoExecuteXmlReader(oracleCommand);
		...
		public static OracleCommand CheckIfOracleCommand(DbCommand command)
		{
			...
			if (oracleCommand == null)
				throw new ArgumentException(HBB.DataContext.Properties.Resources.ExceptionCommandNotOracleCommand, "command");
      ...
      catch (Exception e)
      {
		...
		private void OnOracleRowUpdated(object sender, OracleRowUpdatedEventArgs args)
		{
			if (args.RecordsAffected == 0)
			{
				if (args.Errors != null)
				{
					args.Row.RowError = HBB.DataContext.Properties.Resources.ExceptionMessageUpdateDataSetRowFailure;
		...
		private string TranslatePackageSchema(string storedProcedureName)
		{
			...
			{
				foreach (HBB.DataContext.IOraclePackage oraPackage in packages)
				{
