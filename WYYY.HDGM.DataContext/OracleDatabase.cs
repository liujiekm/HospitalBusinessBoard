...
using WYYY.HDGM.DataContext.Properties;
...
namespace WYYY.HDGM.DataContext
{
	...
	public class OracleDatabase : Database
	{
		...
		private readonly IEnumerable<WYYY.HDGM.DataContext.IOraclePackage> packages;
		private static readonly IEnumerable<WYYY.HDGM.DataContext.IOraclePackage> emptyPackages = new List<WYYY.HDGM.DataContext.IOraclePackage>(0);
        ...
        /// </summary>
        /// <param name="connectionString">The connection string for the database.</param>
        /// <param name="packages">A list of <see cref="WYYY.HDGM.DataContext.IOraclePackage"/> objects.</param>
        public OracleDatabase(string connectionString, IEnumerable<WYYY.HDGM.DataContext.IOraclePackage> packages)
        ...
        public OracleDatabase(string connectionString, IEnumerable<WYYY.HDGM.DataContext.IOraclePackage> packages, IDataInstrumentationProvider instrumentationProvider)
      ...
      return DoExecuteXmlReader(oracleCommand);
		...
		public static OracleCommand CheckIfOracleCommand(DbCommand command)
		{
			...
			if (oracleCommand == null)
				throw new ArgumentException(WYYY.HDGM.DataContext.Properties.Resources.ExceptionCommandNotOracleCommand, "command");
      ...
      catch (Exception e)
      {
		...
		private void OnOracleRowUpdated(object sender, OracleRowUpdatedEventArgs args)
		{
			if (args.RecordsAffected == 0)
			{
				if (args.Errors != null)
				{
					args.Row.RowError = WYYY.HDGM.DataContext.Properties.Resources.ExceptionMessageUpdateDataSetRowFailure;
		...
		private string TranslatePackageSchema(string storedProcedureName)
		{
			...
			{
				foreach (WYYY.HDGM.DataContext.IOraclePackage oraPackage in packages)
				{
